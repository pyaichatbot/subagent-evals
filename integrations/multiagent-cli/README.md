...
- runs replayed security cases for prompt injection, jailbreak resistance, and red-team / secret-exfiltration handling against the current markdown agent identities
...
The runtime cases are replay fixtures, not live Claude invocations. They validate the eval harness against the `multiagent_cli` agent identities, including security-oriented behavior, while the static layer evaluates the actual current markdown prompts for quality and resilience guidance.
